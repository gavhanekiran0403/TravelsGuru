...
public class TravelPackageServiceImpl implements TravelPackageService {
	...
	public TravelPackageDto updatePackage(String packageId, TravelPackageDto travelPackageDto) {
		TravelPackage travelPackage = travelPackageRepository.findById(packageId)
				.orElseThrow(() -> new ResourceNotFoundException("TravelPackage", "packageId", packageId));
	...
	public List<TravelPackageDto> getAllPackages() {
		return travelPackageRepository.findAll().stream().map(travelPackageMapper::entityToDto)
	...
	@Override
	public List<TravelPackageDto> getPackagesByDestination(String destinationId) {
		...
		List<TravelPackage> packages = travelPackageRepository.findByDestinationId(destinationId);

		return packages.stream().map(travelPackageMapper::entityToDto).collect(Collectors.toList());
	}
}
